...
const Trading = () => {
  ...
  const switchInput = () => {
    setInputToken((prevState) => (prevState === "ETH" ? "NFT" : "ETH"));
  };
    ...
    <Loading isLoading={isLoading}>
      <div className="container">
        ...
        <button onClick={swapTokens} className="cool-button">
          Trade {inputToken} for {inputToken === "ETH" ? "NFT" : "ETH"}
        </button>
        {inputAmount && (
          <p>
            You'll get around {getApproximateTradedAmount()}{" "}
            {inputToken === "ETH" ? "NFT" : "ETH"}
          </p>
        )}
        <p className="switch-input" onClick={switchInput}>
          I want to give {inputToken === "ETH" ? "NFT" : "ETH"} instead!
        </p>
      </div>
    </Loading>
  );
